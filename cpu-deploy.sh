...
minikube image load bert-inference:v1
...
kubectl create deploy bert-inference-deployment --image=bert-inference:v1
...
kubectl expose deploy/bert-inference-deployment --name=bert-inference-deployment --target-port=8888 --port=1234
echo "Deployment exposed as a service."

# Access the service using Minikube
echo "Accessing the service using Minikube..."
minikube service bert-inference-deployment
...
if [ -n "$2" ]; then
  ...
  kubectl scale deploy/bert-inference-deployment --replicas="$2"
